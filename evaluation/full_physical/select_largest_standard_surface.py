...
def load_candidate_rows(candidate_root: Path) -> list[dict]:
    rows = []
    for summary_path in sorted(candidate_root.rglob("summary.json")):
        try:
            summary = json.loads(summary_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            continue
        if "target_s" not in summary or "accepted_for_downstream" not in summary:
            continue
    ...
    recorded_levels = {row["target_s"] for row in rows}
    for candidate_dir in sorted(candidate_root.glob("s_*")):
        if not candidate_dir.is_dir() or not (candidate_dir / "gpu_postflight.csv").is_file():
            continue
        try:
    ...
    for row in rows:
        if row["target_s"] in unique:
            raise ValueError(f"duplicate candidate level s={row['target_s']}")
        unique[row["target_s"]] = row
    return sorted(unique.values(), key=lambda row: row["target_s"])
