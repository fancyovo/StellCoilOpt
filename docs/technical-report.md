...
**关键词：** 仿星器线圈；准螺旋对称；直场线坐标；微分 QS；Flow Matching；无梯度优化；CUDA
...
![图 1：从 Fourier 线圈到体诊断、优化目标和独立物理验收的总体流程。](assets/technical-report/01-pipeline.svg)
...
$y_j(t)$ 和 $z_j(t)$ 采用同一约定，因此每根基本线圈由 99 个几何系数和一个电流描述。程序通过场周期旋转和仿星器对称生成实际参与 Biot--Savart 积分的完整线圈组。每根完整线圈被离散为 256 个直线电流元；对任意一批空间点，CUDA 内核一次返回磁场 $\boldsymbol B$ 及其空间梯度 $\nabla\boldsymbol B$ 。后续的磁轴追踪、 $s$ 与 $\alpha$ 拟合以及微分 QS 计算都只通过这两个场量依赖线圈。
...
![图 2：代表性 QH 样本在 $s_{\mathrm{edge}}=0.36$ 上通过验收的磁通标定。左图红点是 11 个径向层的直接截面平均，蓝线是固定零截距的四次多项式；右图给出 $d\psi/ds$ 。](assets/technical-report/02-flux-calibration.png)
...
![图 3：独立评估、严格续接评估和近邻批量代理的计算依赖。](assets/technical-report/03-evaluation-modes.svg)

图 3 按物理阶段比较三种模式。蓝绿色表示正式重算；橙色表示当前候选的近邻迭代近似；灰色表示从中心继承或一阶近似。

**独立评估。** 输入只有线圈和 $N_{\mathrm{FP}}$ 。程序全局搜索磁轴，并完整计算所有下游量。该模式用于数据集评分、随机起点筛选和最终独立全局重评分。
...
速度网络是 30333540 参数的非因果 Transformer：8 层、宽度 512、8 个注意力头、SwiGLU 中间宽度 1408、PreNorm 与 RMSNorm。线圈 token 采用无位置旋转编码的集合式注意力；线圈掩码支持 1--5 根基本线圈， $N_{\mathrm{FP}}$ 通过条件嵌入注入每层。
...
![图 4：309 条轨迹采用的起点筛选与 64 方向有限差分 Adam。](assets/technical-report/04-optimization-loop.svg)
...
端点状态完整有效，且梯度尺度通过滚动中位数与中位绝对偏差（median absolute deviation，MAD）检查时，程序生成候选更新；新中心随后执行严格续接评估。中心失效会触发有界回退与状态回滚。当前点、截至当步的最佳点、Adam 两个矩、方向随机数状态和预取端点状态分别保存，从而支持精确续跑。
...
性能实验使用冻结的同一实现、同一批端点和单张空闲 RTX 5090。独立与严格续接模式各重复正式评分；近邻模式测试 batch size 2、8、32、64 和 128。每次运行在启动前检查显存、GPU 利用率和计算进程，完成后确认 GPU 资源已经释放。

数值口径固定为 $48^3$ 的 $s$ 拟合网格、三次 $\iota(u)$ 、连续边界模式、一个场周期边界追踪、128 个边界角点和每周期 400 步。附录 C 汇总公开复现所需的固定协议与外部依赖。
...
![图 5：随机 Flow 候选、32 选 1 起点、Adam 最优点和 QUASR QH 的分数比较。](assets/technical-report/05-score-distributions.png)

图 5 含四个相互补充的面板。左上是四组样本的密度直方图：随机 Flow 样本在零分附近有明显失败峰，32 选 1 把起点集中到 65--85 分，Adam 最优点进一步集中到 90--93 分。右上是上尾生存曲线，直接显示任意阈值以上的样本比例；Adam 在 85--92 分区间显著高于 QUASR 对照，QUASR 保留更高的最优尾部。左下逐轨迹比较起点与在线最优点，绝大多数点位于对角线上方。右下比较在线续接最优分和独立全局重评分，数据紧贴对角线，说明高分尾部可由全局轴搜索复现。
...
![图 6：309 条轨迹的分数动力学、最优步位置、增益分布和线圈数成本。](assets/technical-report/06-optimization-dynamics.png)

图 6 左上给出 309 条在线分数的中位数、四分位带和 P10--P90 带。前 25 步贡献主要增益，之后中位数持续缓慢提高。右上统计每条轨迹首次达到截至该步最佳值的步数；43.7% 的最优点位于最后 10 步，57.6% 位于最后 25 步，说明 200 步代表中程预算。左下是最优点相对起点的增益分布，中位增益为 12.416；96.4% 的独立重评分结果保持正增益。右下按基本线圈数统计整条轨迹墙钟，成本随线圈数增加，主要来自 Biot--Savart 场计算和 Flow token 数增加。
...
![图 7：Flow 对局部搜索景观的作用。左图比较正式分数下降 5 分时的线圈位置 RMS 扰动半径；点在对角线上方表示潜空间允许更大的物理扰动。右图比较离散二阶差分粗糙度；点在对角线下方表示潜空间更平滑。每种颜色均含 12 个匹配方向。](assets/technical-report/07-flow-landscape.png)
...
![图 8：309 个同起点配对优化结果。左图每个点是一条轨迹，位于对角线上方表示潜空间得到更高的在线正式中心最佳分，颜色表示共同起始分数。右图给出逐轨迹差值“潜空间最佳分减原参数最佳分”的分布、其中位数及 95% 样本自助法区间。](assets/technical-report/08-flow-vs-data.png)
...
![图 9：Flow 先验的有限筛选能力。左图逐条件比较两种先验各自 32 个候选中的最高分，位于对角线上方表示 Flow 起点更好。右图分别给出单候选有效率、32 个候选中能否找到有效起点，以及最终起点达到 50 分的条件比例。](assets/technical-report/09-flow-initialization.png)
...
![图 10：体微分 QH 到 Simsopt 标准面 QH 的定量标定。散点只包含严格通过标准面求解与独立验收的样本；黑线是正文给出的双对数最小二乘关系。左图为固定探针面，右图为每个位形的自适应边界面。](assets/technical-report/10-volume-surface-calibration.png)

图 10 的点云跨越约五个数量级并沿拟合线保持清晰单调关系。0.944--0.958 的 Spearman 相关支持体指标在优化内环中的排序作用；经验幂律把两种数值口径连接起来，标准磁面求解继续给出候选的最终面误差。

![图 11(a)：标准固定探针面 QH 随优化墙钟的变化。左图细线对应单条轨迹，粗线和阴影为各阶段严格通过样本的中位数与 P10--P90；右图以全部 96 条轨迹为分母，给出各阈值的严格通过比例。](assets/technical-report/11-face-qs-time.png)
...
![图 11(b)：GPU 联合拟合 $\iota$ 与标准磁面 $\iota$ 的比较。左图逐面比较，中央和右侧分别给出绝对误差与相对误差分布。](assets/technical-report/12-iota-calibration.png)
...
![图 12：当前评估器的模式基准。左上为两种正式模式的延迟，右上为近邻批量均摊时间，左下比较近邻总分与严格续接总分，右下单独比较线圈工程分量的一阶代理与精确重算。](assets/technical-report/13-evaluator-benchmark.png)
...
![图 13：两个正式模式的互斥阶段平均耗时分解。](assets/technical-report/14-formal-stage-timing.png)
...
![图 14：代表性线圈组与样本自适应选择的较大磁面。](assets/technical-report/15-coils-surface.png)

![图 15：标准磁面上的白底彩色 $|B|$ 等高线。颜色表示磁场强度。](assets/technical-report/16-boozer-contours.png)

图 15 中等高线沿 QH 螺旋方向近似平直，QA 与 QP 误差比 QH 高约三个数量级。Poincare 截面和稠密残差同时确认该边界对应嵌套磁面。DESC 初态和终态保持嵌套， $\iota(\rho)$ 从约 1.444 平滑增至 1.456；50 步内归一化力残差降低约三个数量级，求解在步数上限处停止。公开报告保留图 14 和图 15 作为代表性输出；逐步运行产物、交互式 HTML 和中间数组不随仓库分发。
...
公开仓库包含 C++/CUDA 评分器、Python 结构化接口、Flow 模型与训练代码、309 条轨迹所用的默认优化入口、完整物理验收入口，以及本文引用的汇总图。QUASR 数据、训练完成的模型权重和逐次运行产生的原始数组不随仓库分发，使用者需要自行准备有授权的数据和检查点。

为避免不同实验口径混用，复现时至少应固定下表中的参数。

| 实验 | 固定协议 |
|---|---|
| 正式体评分 | $48^3$ 的 $s$ 拟合网格；三次 $\iota(u)$ ；连续边界；1 周期、128 角点、400 步；6 次磁通二分 |
| Flow 模型 | 8 层、宽度 512、8 个注意力头、SwiGLU 中间宽度 1408；采用 EMA 参数 |
| 309 条 QH 优化轨迹 | 每条先独立评价 32 个 Flow 候选；200 步；每步 64 个新正交方向的中心差分； $h=0.005$ ； $\eta=0.02$ ； $\beta_1=0.7$ ； $\beta_2=0.999$ ；FP32 RK4-128 |
| 原参数配对对照 | 与潜空间复用相同物理起点、方向种子、步数和评分器； $h=0.0025$ ； $\eta=0.01$ ； $\beta_1=0.7$ ； $\beta_2=0.999$ |
| 起点对照 | 覆盖 48 组 $(N_{\mathrm{FP}},n_c)$ 条件；每组分别评价 32 个 Flow 候选和 32 个逐坐标独立高斯候选 |
| 局部景观 | 3 个中心、每中心 4 个匹配方向、每方向 31 个双侧位置；Flow 使用 FP32 RK4-256 |

公开接口会在 JSON 中保存请求配置、状态、分数组成、诊断量和逐阶段耗时。重新运行实验时还应记录数据划分、模型训练步、软件版本、GPU 型号和随机种子；这些运行身份由使用者自己的实验清单管理，不写死在报告或源代码路径中。
