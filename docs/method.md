...
独立样本使用全局搜索。局部优化端点可以传入中心样本的轴提示；严格延续模式只在提示邻域内寻找同一分支，失败时返回 `branch_lost`。配置值 1 在混合精度 Newton 后用五条 FP64 轨迹复核中心闭合残差与四个扰动方向；当前优化默认的配置值 2 复用 Newton 闭合残差，并以四条 mixed 轨迹保留 Poincare Jacobian、椭圆性和截面形状检查。两种严格模式都保留闭合残差、椭圆拓扑和提示距离拒绝门，也都禁止失败后回退到全局磁轴搜索。
...
原生快速 score 当前使用 $48\times48\times48$ 的均匀 $R/Z/\phi$ 拟合网格，并固定使用 4000 个独立验证点。通用 Python 拟合入口保留更高分辨率默认值，供离线诊断使用；两类入口的默认点数不应混淆。
...
ABI 10 当前默认取 $K=3$，即以 $u=\rho^2=\psi/\psi_{\rm edge}$ 的三次多项式表达磁面上的旋转变换。`iota_degree=0` 仍可显式配置，但不再是生产 score 默认值。
...
- 默认原生路径使用 $48\times48\times48$ 的 $\psi$ 拟合网格、4000 个独立 $\psi$ 验证点、固定 100000 个体点、30000 个 $\alpha$ 拟合点、三次 $\iota(\rho^2)$ 和固定候选/追踪预算。配置可改，但结果必须记录完整配置。
